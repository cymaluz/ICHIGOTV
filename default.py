...
CHBase = base64.decodestring('aHR0cDovL3RleHR1cGxvYWRlci5jb20vYWpwMXAvcmF3')
...
addon = xbmcaddon.Addon('plugin.video.brasiliptvonline')
...
dialog.ok("BrasilIPTVOnLine", "                         Bem-vindos ao [COLOR green]Brasil[/COLOR][COLOR yellow]IPTV[/COLOR][COLOR blue]On[/COLOR][COLOR white]Line[/COLOR].                 Assistam a vários canais IPTV do Brasil e do mundo!!!")'''
	
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.BrasilIPTVOnLine-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(BrasilIPTVOnLine,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(BrasilIPTVOnLine,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")

				
def CHIndex():
    addon_log("CHIndex")
    addDir('[B][COLOR orange] Buscar Listas[/B][/COLOR] ','Search',40,'https://copy.com/myEaTELQ8aomsfeR' ,  FANART,'','','','')
    getData(CHBase,'')
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(BrasilIPTVOnLine,New source added.,5000,"+icon+")")
        if not url is None:
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         dialog=xbmcgui.Dialog()
         rNo=dialog.select('BrasilIPTVOnLine Select A Source', List)
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
...
#hakamac code
    ...
    if keyboard.isConfirmed():
       ...
       if len(Searchkey) == 0: 
          xbmcgui.Dialog().ok('BrasilIPTVOnLine', 'Nothing Entered')
          return	   
    ...
    progress.create('BrasilIPTVOnLine Searching Please wait',' ')
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(BrasilIPTVOnLine,Please enable Update Commonresolvers to Play in Settings. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification(BrasilIPTVOnLine,Using Urlresolver module.. - ,5000)")
        import urlresolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('BrasilIPTVOnLine','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('BrasilIPTVOnLine', 'Do you want to add this file as a source?')
        if ret:
...
def addDir(name,url,mode,iconimage,fanart,description,genre,date,credits,showcontext=False):
        ...
        if showcontext:
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from BrasilIPTVOnLine Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                                    %(sys.argv[0], urllib.quote_plus(name))))
									
            if not name in FAV:
                contextMenu.append(('Add to BrasilIPTVOnLine Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification(BrasilIPTVOnLine,No IMDB match found ,7000,"+icon+")")
...
def addLink(url,name,iconimage,fanart,description,genre,date,showcontext,playlist,regexs,total,setCookie=""):
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from BrasilIPTVOnLine Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to BrasilIPTVOnLine Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
if mode==None:
    ...
    if url:
        ...
        xbmc.executebuiltin("XBMC.Notification(BrasilIPTVOnLine,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(BrasilIPTVOnLine,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
